<template>
    <Head :title="title" />
    <div class="card">
        <div class="card-body" :style="{ height: (height+58) + 'px' }">
            ...
            <div class="table-responsive" v-else>
                ...
                <SimpleBar :style="{ height: (height - 105) + 'px' }">
                    <table class="table table-centered table-nowrap mb-0">
                        <tbody>
                            <tr v-for="user in users" v-bind:key="user.username" :id="user.username">
                                <td style="cursor: pointer;" @click="profile(user)">
                                    <h5 class="font-size-11 mb-0 text-dark">{{user.lastname}}, {{user.firstname}} {{user.middlename}}</h5>
                                    <p class="font-size-11 text-muted mb-0">{{user.username}}</p>
                                </td>
...
export default {
    ...
    methods : {
        ...
        preview(){
            ...
            axios.post(this.currentUrl + '/user/import', data)
            .then(response => {
